...
func main() {
	...
	generateYAML(requiredGoVersion, latestRelease, versionMessage, buildTime)
	fmt.Println("==> Generated go-yaml.yaml")
	...
	fmt.Println("    Visit: http://localhost:1313/go-yaml/")
	fmt.Println()

	// Start Hugo server with both configs.
	cmd := exec.CommandContext(ctx, "hugo", "server",
		"--config", "hugo.yaml,go-yaml.yaml,metrics.yaml",
		"--buildDrafts",
		"--disableFastRender",
		"--navigateToChanged",
		"--bind", "0.0.0.0",
		"--port", "1313",
		"--baseURL", "http://localhost:1313/go-yaml/",
		"--appendPort=false",
		"--logLevel", "info",
		"--cleanDestinationDir",
	)
...
// generateYAML reads the template and writes go-yaml.yaml with substitutions.
func generateYAML(goVersion, latestRelease, versionMessage, buildTime string) {
	tmpl, err := os.ReadFile("go-yaml.yaml.template")
	if err != nil {
	...
	if err := os.WriteFile("go-yaml.yaml", []byte(out), 0o600); err != nil { //nolint:mnd
		fatalf("writing go-yaml.yaml: %v", err)
	}
}
