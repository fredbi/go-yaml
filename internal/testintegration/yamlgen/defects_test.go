...
// TestDefectAnAliasKeyEscapesTheDuplicateCheck pins what the two paths do with
// one node used twice as a key.
//
// 3.2.2.2 makes an alias node the anchored node rather than a copy of it, so
// "&a1 x: 1" over "*a1 : 2" writes one key twice and 3.2.1.1 refuses it. The
// walk reads it and keeps the last entry, losing the first with nothing
// reported. The tree refuses it where the key names a scalar -- so the check
// exists and one path skips it, which is what makes this actionable rather than
// a position.
//
// The `.inf` pair is the sharpest of them, because the two paths disagree about
// how many keys the mapping has: the walk gives one entry and the tree gives
// *two*, naming the anchored key ".inf" and the alias key "+Inf". Two faults
// meeting -- this one and
// TestDefectAFloatKeyBehindAPropertyLosesItsSpelling -- and the second hides
// the first from the check that would have caught it.
//
// The controls place it on the alias and nothing else. The same two keys
// written out in full are refused, and so are an anchored key and a written-out
// one: only the alias escapes.
//
// The peer session's keepAnchorIdentity records what an anchored node resolves
// to and checks an alias key against it. Asked for by name on 2026-09-08:
// reachable is not guarded, since a regeneration reshuffles every seed and a
// shape the corpus finds today can be absent tomorrow with nothing failing.
func TestDefectAnAliasKeyEscapesTheDuplicateCheck(t *testing.T) {
	t.Run("the same key written out twice is refused", func(t *testing.T) {
		for _, src := range []string{
			".inf: 1\n.inf: 2\n",
			// An anchor on one of them changes nothing, which places the fault
			// on the alias rather than on properties in general.
			"&a1 .inf: 1\n.inf: 2\n",
		} {
			var got any
			err := codec.Unmarshal([]byte(src), &got)
			require.Errorf(t, err, "%q", src)
			assert.Containsf(t, err.Error(), `mapping key ".inf" already defined`, "%q", src)
		}
	})

	t.Run("today the walk reads an alias key repeating one and drops a value", func(t *testing.T) {
		for _, tc := range []struct {
			src  string
			want map[string]any
		}{
			{src: "k: &a1 n\n*a1 : 1\nn: 2\n", want: map[string]any{"k": "n", "n": uint64(2)}},
			{src: "{&a1 x: 1, *a1 : 2}\n", want: map[string]any{"x": uint64(2)}},
		} {
			var walked any
			require.NoErrorf(t, codec.Unmarshal([]byte(tc.src), &walked), "%q", tc.src)
			assert.Equalf(t, tc.want, walked, "today: the last entry wins and the first is gone: %q", tc.src)

			// The tree does refuse it, so the check exists and the walk skips
			// it.
			var tree map[string]any
			err := codec.Unmarshal([]byte(tc.src), &tree)
			require.Errorf(t, err, "the tree refuses it: %q", tc.src)
			assert.Containsf(t, err.Error(), "duplicate key", "%q", tc.src)
		}
	})

	t.Run("today an anchored .inf key and an aliased one are two keys to the tree", func(t *testing.T) {
		// The two paths disagree about how many keys the mapping holds, which
		// no other shape in this file does. The walk names both "+Inf" and
		// keeps the last; the tree names the anchored one ".inf" and the alias
		// "+Inf" and holds both.
		const src = "&a1 .inf: 1\n*a1 : 2\n"

		var walked any
		require.NoError(t, codec.Unmarshal([]byte(src), &walked))
		assert.Equal(t, map[string]any{"+Inf": uint64(2)}, walked,
			"today: one entry on the walk, and the 1 is gone")

		var tree map[string]any
		require.NoError(t, codec.Unmarshal([]byte(src), &tree))
		assert.Equal(t, map[string]any{"+Inf": uint64(2), ".inf": uint64(1)}, tree,
			"today: two entries on the tree, named two ways")
	})

	t.Run("and two distinct anchored collections as keys are two keys, correctly", func(t *testing.T) {
		// The accepting control, and the direction that fails loudly: a checker
		// naming a key from a scrubbed node makes both of these the same key
		// and refuses a valid document. The walk reads all four entries.
		const src = "a: &x [1,2]\nb: &y [3,4]\n*x : p\n*y : q\n"

		var walked any
		require.NoError(t, codec.Unmarshal([]byte(src), &walked))
		assert.Equal(t, map[string]any{
			"a":     []any{uint64(1), uint64(2)},
			"b":     []any{uint64(3), uint64(4)},
			"[1 2]": "p",
			"[3 4]": "q",
		}, walked)
	})
