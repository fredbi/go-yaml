...
// firstUnprintableByByte is the loop firstUnprintable replaced: one byte at a
// time, decoding a rune wherever the byte is not ASCII. It is kept as the
// reference the word-at-a-time scan is held to.
...
func TestFirstUnprintableMatchesTheByteLoop(t *testing.T) {
	same := func(t *testing.T, text string) {
		t.Helper()
		assert.Equalf(t, firstUnprintableByByte(text), firstUnprintable(text),
			"the two disagree on %q", text)
	}

	for _, doc := range workloadDocsInternal(t) {
		same(t, doc)
	}

	// A byte a stream may not hold, at every offset of a run long enough to
	// cross several words, with and without a multi-byte character in front of
	// it so that the scan reaches it both ways.
	...
	// Text a stream may hold, at the same offsets, so that a false alarm shows
	// up as loudly as a missed one.
