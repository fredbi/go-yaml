...
// extents and returns "" where they break, and the loop below skips an empty want. extentLedger in extent_test.go
// records such documents and holds none: the four it held until a plain scalar continued by a "- " line started where
// its text does broke on their last token, so no token after one had its origin shifted into a miss.
//
