...
var parseModes = map[string]refparser.Mode{
	"default":  0,
...
func FuzzParserParseBytes(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, src string) {
		for name, mode := range parseModes {
			file, err := refparser.ParseBytes([]byte(src), mode)
			if err != nil {
			...
			again, err := refparser.ParseBytes([]byte(src), mode)
...
func FuzzParserWalk(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, src string) {
		file, err := refparser.ParseBytes([]byte(src), refparser.ParseComments)
		if err != nil {
			return
		}
