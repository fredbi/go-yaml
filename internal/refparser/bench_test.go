...
func BenchmarkParseBytes(b *testing.B) {
	corpus.ForEachDocument(b, func(b *testing.B, src []byte) {
		for b.Loop() {
			if _, err := refparser.ParseBytes(src, 0); err != nil {
				b.Fatal(err)
			}
		}
	})
...
func BenchmarkParseBytesWithComments(b *testing.B) {
	corpus.ForEachDocument(b, func(b *testing.B, src []byte) {
		for b.Loop() {
			if _, err := refparser.ParseBytes(src, refparser.ParseComments); err != nil {
				b.Fatal(err)
			}
		}
	})
...
// grouping, and BenchmarkParserParse adds the tree. refparser.New reads an
...
func BenchmarkRender(b *testing.B) {
	corpus.ForEachDocument(b, func(b *testing.B, src []byte) {
		file, err := refparser.ParseBytes(src, refparser.ParseComments)
