...
func TestDuplicateMapKeyIsReportedPerMapping(t *testing.T) {
	...
	for name, test := range tests {
		t.Run(name, func(t *testing.T) {
			_, err := refparser.ParseBytes([]byte(test.src), 0)
			if !test.duplicate {
...
func TestDuplicateMapKeyAllowed(t *testing.T) {
	_, err := refparser.ParseBytes([]byte("foo: 1\nfoo: 2\n"), 0, refparser.AllowDuplicateMapKey())
	require.NoError(t, err)
}
