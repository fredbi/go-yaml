> [!CAUTION]
> **Superseded — archived 2026-08-27.** Written as an ask to `goccy/go-yaml`, for an audience that
> no longer exists: the fork is now a hard fork and nothing here will be sent upstream. Kept for the
> reproducers, every one of which was run against v1.19.2 (`edee2f9`).
>
> The measurements survive in `ANALYSIS-go-openapi.md` and in
> [`reference/upstream-issues.md`](../reference/upstream-issues.md). The live plans are in
