...
func TestDuplicateMapKeyIsReportedPerMapping(t *testing.T) {
	...
	for name, test := range tests {
		t.Run(name, func(t *testing.T) {
			_, err := parser2.ParseBytes([]byte(test.src), 0)
			if !test.duplicate {
...
func TestDuplicateMapKeyAllowed(t *testing.T) {
	_, err := parser2.ParseBytes([]byte("foo: 1\nfoo: 2\n"), 0, parser2.AllowDuplicateMapKey())
	require.NoError(t, err)
}
