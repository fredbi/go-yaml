...
var parseModes = map[string]parser2.Mode{
	"default":  0,
	"comments": parser2.ParseComments,
}
...
func FuzzParserParseBytes(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, src string) {
		for name, mode := range parseModes {
			file, err := parser2.ParseBytes([]byte(src), mode)
			if err != nil {
			...
			again, err := parser2.ParseBytes([]byte(src), mode)
...
func FuzzParserWalk(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, src string) {
		file, err := parser2.ParseBytes([]byte(src), parser2.ParseComments)
		if err != nil {
			return
		}
