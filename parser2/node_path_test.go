...
func paths(t *testing.T, src string, opts ...parser2.Option) []string {
	t.Helper()

	f, err := parser2.ParseBytes([]byte(src), parser2.ParseComments, opts...)
...
func TestOmitNodePathsSilencesGetPath(t *testing.T) {
	...
	with, err := parser2.ParseBytes([]byte(src), parser2.ParseComments)
	require.NoError(t, err)
	without, err := parser2.ParseBytes([]byte(src), parser2.ParseComments, parser2.OmitNodePaths())
	require.NoError(t, err)
	...
	for _, p := range paths(t, src, parser2.OmitNodePaths()) {
		assert.Empty(t, p)
	}
}
...
func TestSetPathOverridesTheRecordedPath(t *testing.T) {
	f, err := parser2.ParseBytes([]byte("foo: 1\n"), 0)
	require.NoError(t, err)
