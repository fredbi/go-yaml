...
func TestParseBlockScalarAtTheDocumentRoot(t *testing.T) {
	...
	t.Run("not under a mapping key", func(t *testing.T) {
		_, err := parser2.ParseBytes([]byte("a: |\nb\n"), parser2.ParseComments)
		assert.Error(t, err)
	})
}
...
func TestParseEmptyDocumentsKeepTheirStream(t *testing.T) {
	...
	for name, test := range tests {
		t.Run(name, func(t *testing.T) {
			...
			for _, mode := range []parser2.Mode{0, parser2.ParseComments} {
				file, err := parser2.ParseBytes([]byte(test.source), mode)
...
func TestParseDocumentsAfterASuffix(t *testing.T) {
	...
	for name, source := range valid {
		t.Run(name, func(t *testing.T) {
			file, err := parser2.ParseBytes([]byte(source), parser2.ParseComments)
